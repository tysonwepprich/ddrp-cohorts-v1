...
agg_res <- 1
fs <- list.files("daymet/orders", pattern = '.nc4', full.names = TRUE, recursive = TRUE)
...
outfiles <- foreach(f = 1:length(fs),
  ...
  foreach(day = 1:365,
          ...
          .inorder = FALSE)%dopar%{
            ...
            ifelse(!dir.exists(file.path("daymet_hi", yr)), dir.create(file.path("daymet_hi", yr)), FALSE)
            
            
            newname <- stringr::str_split_fixed(string = newname, pattern = "_", n = 3)[,1:2]
            # for CONUSPLUS
            # e_2 <- extent(-131, -52, 24, 55)
            ...
            prism_res <- 25.875 / 621 / 4 # divided by 4 for Daymet 1 x 1
            ...
            fname <-  paste0("/home/tyson/REPO/ddrp-cohorts-v1/daymet_hi/", newname[2], "/DAYMET_", varname, "_stable_1kmD1_", dat, ".grd")
